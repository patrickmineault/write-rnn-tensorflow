...
class Model():
  def __init__(self, args, infer=False):
    ...
    outputs, state_out, all_states = rnn_decoder(inputs, self.state_in, cell, loop_function=None, scope='rnnlm')
    ...
    self.all_states = tf.stack(all_states, axis=1)
    self.state_mean, self.state_var = tf.nn.moments(self.all_states, axes=[1])
